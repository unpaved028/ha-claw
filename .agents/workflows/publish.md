...
// turbo
3.  **Änderungen committen**: Führe die Git-Commands aus, um die Code-Änderungen inkl. Versions-Bump zu speichern. Ersetze `<VERSION>` durch die angegebene Version (z. B. `v0.6.3`).
    ```bash
    git add .
...
// turbo
4.  **Git Tag erstellen**: Erstelle einen neuen Tag für die Version.
    ```bash
    git tag <VERSION>
    ```

// turbo
5.  **Pushen auf GitHub**: Lade die Änderungen und Tags auf GitHub hoch.
    ```bash
    git push origin main
    git push origin --tags
    ```

6.  (Optional) Falls das `gh` CLI installiert ist und ein formaler GitHub Release erstellt werden soll, kannst du dies mit `gh release create <VERSION> --generate-notes` tun.
7.  Zeige dem User eine Erfolgsmeldung, dass der Release nun gepublisht ist!
